...
const partials = require('../server/partials');
...
app.post('/datas/login', async (req, res) => {
  try {
    ...
    const response = await Collection.findByCredentials(
      loginDetails.name,
      loginDetails.pass
    );
    const token = await response.collection.generateAuthToken(
      response.loginType
    );
    res.header('x-auth', token).send({
      ...
      token: token,
      html: { user: partials.user, admin: partials.admin }
    });
  } catch {
    res.status(400).send();
  }
...
app.patch('/datas/logout', authenticate, async (req, res) => {
  try {
    ...
    res.status(400).send('loyugout error');
  }
});
